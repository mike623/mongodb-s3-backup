...
if [[ -z $MONGODB_USER ]] || [[ -z $MONGODB_PASSWORD ]] || [[ -z $AWS_ACCESS_KEY ]] || [[ -z $AWS_SECRET_KEY ]] || [[ -z $S3_REGION ]] || [[ -z $S3_BUCKET ]] || [[ -z $AUTH_SERVER ]] || [[ -z $MONGO_DB ]]
then
  usage
  exit 1
fi
...
# Dump the database
mongodump -username "$MONGODB_USER" -password "$MONGODB_PASSWORD" --authenticationDatabase "$AUTH_SERVER" --db "$MONGO_DB" --excludeCollection system.users --out $DIR/backup/$FILE_NAME
